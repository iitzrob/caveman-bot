...
async function performTicketClose(guild, channel, closedByUser, reason, opts = {}) {
  ...
  deleteOriginalCategory(channel.id);
...
client.on("interactionCreate", async i => {
  ...
  if (i.isButton() && (i.customId === "claim" || i.customId === "close" || i.customId === "unclaim")) {
    ...
    if (i.customId == "claim") {
      if (isService) {
        // Move the ticket out of digout/basebuilding (or whatever service
        // category it's in) and into the shared "claimed" category, so
        // staff can find in-progress tickets in one place. Remember where
        // it came from so unclaim can put it back. Do this before the
        // permission overwrites below, since moving categories can reset
        // overwrites if lockPermissions isn't explicitly disabled.
        if (config.serviceClaimedCategory) {
          setOriginalCategory(i.channelId, i.channel.parentId);
          await i.channel.setParent(config.serviceClaimedCategory, { lockPermissions: false }).catch(err => {
            console.error(`Failed to move claimed ticket ${i.channelId} to serviceClaimedCategory:`, err);
          });
        }
    ...
    if (i.customId == "unclaim") {
      ...
      if (isService) {
        ...
        // Move it back to whichever service category it was claimed out of.
        const originalCategory = getOriginalCategory(i.channelId);
        deleteOriginalCategory(i.channelId);
        if (originalCategory) {
          await i.channel.setParent(originalCategory, { lockPermissions: false }).catch(err => {
            console.error(`Failed to move unclaimed ticket ${i.channelId} back to ${originalCategory}:`, err);
          });
        }
      }
